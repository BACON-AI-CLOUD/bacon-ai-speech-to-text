...
| REQ-117 | API keys stored in backend config.json only (never in frontend, never logged, never in URLs) | P0 (Must) |
...
- Keys stored in backend config.json only (never in frontend localStorage)
- Keys submitted to backend via PUT /refiner/config and stored server-side
...
| NFR-103 | API key security | Keys in localStorage only, never logged on backend, never in URLs |
